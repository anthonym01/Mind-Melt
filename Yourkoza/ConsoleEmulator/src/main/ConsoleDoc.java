...
public class ConsoleDoc extends DefaultStyledDocument implements CaretListener {
	private Caret caret;
    ...
    public void write(String text, MutableAttributeSet ats) {
        try {
            insertString(getLength(), text, ats);
            limit = getLength();
            caret.setDot(limit);
    ...
    public void writeUser(String text, MutableAttributeSet ats) {
		try {
			insertString(getLength(), text, ats);
