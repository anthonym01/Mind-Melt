...
public class Console extends JTextPane implements KeyListener, MouseWheelListener, ComponentListener, MouseListener {
	...
	private ConsoleDoc doc;
	private int maxLinesPerScreen = 17;
	private String prompt; //Structure: [connection][subprompt][prompt]
	private String subPrompt = ""; //Subprompt: one character at the end of the prompt
	public String connection = "Home>"; //Computer name, IP, or Domain name
    ...
    private int currentCommandnum = 0; //The current command number, as referenced in "prompts," that the user is accessing, based on arrow keys
	...
	public Console(Color background, Color text, Font font, String prompt) {
        ...
        f = font;
        MutableAttributeSet ats = getInputAttributes();
        StyleConstants.setFontFamily(ats, font.getFamily());
        StyleConstants.setFontSize(ats, font.getSize());
        StyleConstants.setItalic(ats, (font.getStyle() & Font.ITALIC) != 0);
        StyleConstants.setBold(ats, (font.getStyle() & Font.BOLD) != 0);
        StyleConstants.setForeground(ats, text);
        getStyledDocument().setCharacterAttributes(0, doc.getLength() + 1, ats, false);
        defaultStyle = ats;
	...
	// "Clears" the terminal window by replacing it with a new one
	public void cls() {
        doc = new ConsoleDoc();
        doc.setConsole(this);
        setDocument(doc);
	...
	public ConsoleDoc getConsoleDocument() {
		return this.doc;
	}
